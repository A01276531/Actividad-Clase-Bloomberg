...
# File names (adjust for Colab's renaming, e.g., 'AAPL 1min (6).xlsx')
file_names = {
...
# Perform ADF tests
adf_test(aapl_close, 'AAPL')
adf_test(nvda_close, 'NVDA')
adf_test(googl_close, 'GOOGL')

# If non-stationary, difference the series
aapl_diff = aapl_close.diff().dropna() if adfuller(aapl_close)[1] >= 0.05 else aapl_close
nvda_diff = nvda_close.diff().dropna() if adfuller(nvda_close)[1] >= 0.05 else nvda_close
googl_diff = googl_close.diff().dropna() if adfuller(googl_close)[1] >= 0.05 else googl_close
...
# Fit ARMA models (assuming ARMA(1,1) for simplicity)
...
# Fit ARMA models
aapl_arma = fit_arma(aapl_diff, 'AAPL')
nvda_arma = fit_arma(nvda_diff, 'NVDA')
googl_arma = fit_arma(googl_diff, 'GOOGL')
...
coint_test(aapl_close, nvda_close, 'AAPL', 'NVDA')
coint_test(aapl_close, googl_close, 'AAPL', 'GOOGL')
coint_test(nvda_close, googl_close, 'NVDA', 'GOOGL')
...
def forecast_and_plot(model, series, name, steps=10):
    ...
    try:
        ...
        plt.title(f'{name} Closing Price Forecast (10 Minutes Ahead)')
...
print("Forecasts predict the next 10 minutes of closing prices.")
...
def plot_correlograms(series, name, max_lags=20):
    ...
    plt.figure(figsize=(12, 5))
    plt.subplot(1, 2, 1)
    plot_ac [plot_acf(series, lags=nlags, ax=plt.gca())]
    plt.title(f'ACF for {name}')
    plt.subplot(1, 2, 2)
    plot_pacf(series, lags=nlags, ax=plt.gca())
    plt.title(f'PACF for {name}')
